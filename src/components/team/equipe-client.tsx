...
export function EquipeClient({ isAdmin, canManageTeam }: { isAdmin: boolean; canManageTeam: boolean }) {
  ...
  return (
    <div>
            ...
            {members.map((m) => (
              <div key={m.id} className="flex items-center justify-between p-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {m.custom_permissions && <Badge tone="warning">permissões customizadas</Badge>}
                  {m.status !== 'active' && <Badge tone="warning">{m.status}</Badge>}
                  {isAdmin && (
                    <Button size="sm" variant="ghost" onClick={() => setEditingMember(m)} title="Editar permissões">
      ...
      <PermissionsModal member={editingMember} onClose={() => setEditingMember(null)} onSaved={load} />
    </div>
  )
}
...
function PermissionsModal({
  ...
  useEffect(() => {
    if (!member) return
    // eslint-disable-next-line react-hooks/set-state-in-effect -- reseta o form ao trocar de membro
    setRole(member.role)
  ...
  function effectiveValue(key: keyof RolePermissions): boolean {
    return overrides[key] ?? roleDefaults[key]
  }

  function toggle(key: keyof RolePermissions) {
    const next = { ...overrides }
    const newValue = !effectiveValue(key)
    if (newValue === roleDefaults[key]) {
      // volta a bater com o padrão do papel — não precisa mais de override
      delete next[key]
    } else {
      next[key] = newValue
    }
  ...
  async function handleSave() {
    setSaving(true)
    setError(null)
    const res = await fetch(`/api/equipe/${member!.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        role,
        custom_permissions: Object.keys(overrides).length > 0 ? overrides : null,
      }),
    })
    setSaving(false)
    if (!res.ok) {
  ...
  return (
    <Modal open={!!member} onClose={onClose} title={`Permissões — ${member.display_name || member.email}`} className="max-w-lg">
      <div className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-xs text-muted">Define o padrão de permissões. Marcações abaixo sobrescrevem esse padrão.</p>
        </div>
              ...
              return (
                <label key={key} className="flex items-center justify-between gap-2 text-sm">
                  <span className={isOverridden ? 'font-medium' : ''}>
                    ...
                    {isOverridden && <span className="ml-1.5 text-xs text-brand">(customizado)</span>}
                  </span>
                  <input
                    type="checkbox"
        ...
        <div className="flex items-center justify-between pt-2">
          <Button type="button" variant="ghost" onClick={() => setOverrides({})} disabled={Object.keys(overrides).length === 0}>
